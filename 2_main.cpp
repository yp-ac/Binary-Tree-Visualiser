...
void display_bst_node(HDC hdc, HWND hWnd, bst_node_t *p_node, int level, int isLeft);
void display_bst(HDC hdc, HWND hWnd, bst_t *p_bst);
void draw_node(HDC hdc, HWND hWnd, int x, int y, int data);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case ID_DEL:
            MessageBox(hwnd, TEXT("This Functionality is yet to implement!"), TEXT("Delete"), MB_OK);
            break;
...
void display_bst_node(HDC hdc, HWND hWnd, bst_node_t *p_node, int level, int isLeft)
{
    ...
    assert(stack_peek(prev_positions, &prev_x) == SUCCESS);
    ...
    draw_node(hdc, hWnd, x, y, p_node->data);

    stack_push(prev_positions, x);

    display_bst_node(hdc, hWnd, p_node->left, level + 1, TRUE);
    display_bst_node(hdc, hWnd, p_node->right, level + 1, FALSE);
...
void display_bst(HDC hdc, HWND hWnd, bst_t *p_bst)
{
    ...
    draw_node(hdc, hWnd, x, y, p_bst->p_root->data);

    assert(stack_push(prev_positions, x) == SUCCESS);

    display_bst_node(hdc, hWnd, p_bst->p_root->left, 2, TRUE);
    display_bst_node(hdc, hWnd, p_bst->p_root->right, 2, FALSE);
